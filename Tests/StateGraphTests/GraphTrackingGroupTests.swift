...
struct GraphTrackingGroupTests {
  ...
  func basicConditionalTracking() async throws {
    ...
    let cancellable = withGraphTracking {
      withGraphTrackingGroup {
        ...
        print("node1: \(node1.wrappedValue), condition: \(node1.wrappedValue > 10)")
    ...
    try await Task.sleep(nanoseconds: 100_000_000) // 100ms
    print("After node2 change (condition true): callCounter = \(callCounter.count)")
  ...
  func dynamicConditionChanges() async throws {
    ...
    let cancellable = withGraphTracking {
      withGraphTrackingGroup {
        callCounter.increment()
        print("=== Group called, count: \(callCounter.count), condition: \(condition.wrappedValue) > 10 = \(condition.wrappedValue > 10) ===")
    ...
    try await Task.sleep(nanoseconds: 100_000_000) // 100ms
    print("After tracked change (condition true): callCounter = \(callCounter.count)")
    ...
    try await Task.sleep(nanoseconds: 100_000_000) // 100ms
    print("After tracked change (condition false): callCounter = \(callCounter.count)")
    ...
    try await Task.sleep(nanoseconds: 100_000_000) // 100ms
    print("After tracked change (condition true again): callCounter = \(callCounter.count)")
