  ...
  Sendable
{
  ...
  public struct Observations<Element: Sendable, Failure: Error>: AsyncSequence,
    Sendable
  {
    public enum Iteration: Sendable {
      case next(Element)
    ...
    public struct Iterator: AsyncIteratorProtocol {
      ...
      var state: OSAllocatedUnfairLock<State>?
      let emit: Emit
      var started = false
      ...
      public mutating func next(
        ...
        do {
          ...
          if !started {
            started = true
            return try await trackEmission(
