...
## Initializing and Updating Entities with @InitAndUpdate

Normalized entities are often created from a payload and later refreshed from
another payload with the same identity. `@InitAndUpdate` keeps those two paths
in one declaration by generating an in-place `update` method from an
initializer:

```swift
/// The latest server representation of an article.
struct ArticlePayload: Sendable {
  let id: String
  let title: String
  let content: String
}

final class Article: TypedIdentifiable, Sendable {
  typealias TypedIdentifierRawValue = String
  ...
  @GraphStored var refreshCount = 0

  @InitAndUpdate
  init(payload: ArticlePayload) {
    #onInit {
      // Identity is established only when the entity is created.
      self.typedID = .init(payload.id)
    }

    // These assignments are shared by initialization and updates.
    self.title = payload.title
    self.content = payload.content

    #onUpdate {
      // Update-only work runs after the entity already exists.
      self.refreshCount += 1
    }
  }
}
...
let articleID: Article.TypedID = .init(payload.id)

var articleStore = EntityStore<Article>()
articleStore.updateOrCreate(
  id: articleID,
  update: { article in
    article.update(payload: payload) // Generated by @InitAndUpdate
  },
  create: {
...
Statements outside a marker are emitted into both the initializer and the
generated method. Use `#onInit` for initialization-only work such as assigning
immutable identity or establishing relationships, and `#onUpdate` for work that
must happen only when refreshing an existing entity.

For a class, the macro generates `func update`; for a struct, it generates
`mutating func update`. The method preserves the initializer's parameters,
argument labels, access level, effects, generic parameters, and constraints.
The macro does not automatically insert the entity into an `EntityStore` or
choose a merge policy—the caller still decides when an incoming payload should
create an entity or update an existing one. `EntityStore.updateOrCreate` pairs
with the generated method when that decision is based on the entity's typed ID.
