...
extension Optional: UserDefaultsStorable where Wrapped: UserDefaultsStorable {
  public static func getValue(from userDefaults: UserDefaults, forKey key: String, defaultValue: Optional<Wrapped>) -> Optional<Wrapped> {
    if userDefaults.object(forKey: key) == nil {
      return defaultValue
    }
    return Wrapped.getValue(from: userDefaults, forKey: key, defaultValue: defaultValue ?? nil as Wrapped?) as? Wrapped
  }
